...
class VersionTest(unittest.TestCase):
	def test_build_no_injection(self):
		...
		if "CI_PIPELINE_ID" in os.environ:
			self.assertTrue(deep_sea_treasure.__version__.VERSION.endswith(str(os.environ["CI_PIPELINE_ID"])))
			self.assertRegex(deep_sea_treasure.__version__.VERSION, "\\d+\\.\\d+\\.\\d+\\+\\d+")
		else:
